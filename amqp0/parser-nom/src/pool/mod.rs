...
pub trait ParserPool {
    ...
    fn new_table_entries_vec(&mut self, &[u8]) -> Vec<(Cow<'static, str>, Value<'static>)>;
    ...
    fn return_table_entries_vec(&mut self, _: Vec<(Cow<'static, str>, Value<'static>)>) {}
...
impl ParserPool for NoParserPool {
    ...
    fn new_table_entries_vec(&mut self, _: &[u8]) -> Vec<(Cow<'static, str>, Value<'static>)> {
        Vec::with_capacity(10)
    }
}