// Copyright 2016 Alexander Reece
//
// Licensed under the Apache License, Version 2.0 <LICENSE-APACHE or
// http://www.apache.org/licenses/LICENSE-2.0> or the MIT license
// <LICENSE-MIT or http://opensource.org/licenses/MIT>, at your
// option. This file may not be copied, modified, or distributed
// except according to those terms.

mod header_struct;
mod method_struct;
mod method_enum;

use std::io;
use primalgen::Class;
use common::domain::DomainMapper;

use CodeGenerator;

use self::method_enum::MethodEnumWriter;
use self::header_struct::HeaderStructWriter;
use self::method_struct::MethodStructWriter;

pub struct ClassModuleWriter<'a> {
    domain_mapper: &'a DomainMapper<'a>,
    class: &'a Class<'a>,
}

impl<'a> ClassModuleWriter<'a> {
    pub fn new(class: &'a Class, domain_mapper: &'a DomainMapper) -> Self {
        ClassModuleWriter {
            domain_mapper: domain_mapper,
            class: class,
        }
    }
}

impl<'a> CodeGenerator for ClassModuleWriter<'a> {
    fn write_rust_to<W>(&self, writer: &mut W) -> io::Result<()>
        where W: io::Write
    {
        try!(writeln!(writer, "\n// generated by primalgen::codegen::spec_module::class_mod::ClassModuleWriter"));
        try!(writeln!(writer, "pub mod {} {{", self.class.snake_case()));
        let header = HeaderStructWriter::new(self.class, &self.domain_mapper);
        try!(header.write_to(writer));

        for method in self.class.methods() {
            // try!(writeln!(writer, "pub mod {} {{", method.snake_case()));

            // write the struct
            let struct_writer = MethodStructWriter::new(&self.class, &method);
            try!(struct_writer.write_rust_to(writer));

            // try!(writeln!(writer, "}} // mod {}\n", method.snake_case()));
        }

        let method_enum = MethodEnumWriter::new(self.class);
        try!(method_enum.write_rust_to(writer));

        try!(writeln!(writer, "}} // mod {}", self.class.snake_case()));

        Ok(())
    }
}