...
impl<'a> ::Encodable for Blocked<'a> {
    ...
    {
        try!(::Encodable::write_encoded_to(&self.reason, writer)); // reason
...
#[test]
fn test_blocked_encodable_bytes_written_matches_len() {
    let payload: Blocked = Default::default();
...
impl<'a> From<Blocked<'a>> for ClassMethod<'a> {
    fn from(from: Blocked<'a>) -> Self {
        ClassMethod::Blocked(from)
    } // fn from()
} // impl From<Blocked<'a>> for ClassMethod

impl<'a> From<Blocked<'a>> for super::SpecMethod<'a> {
    fn from(from: Blocked<'a>) -> Self {
        super::SpecMethod::Connection(from.into())
    } // fn default()
} // impl From<Blocked<'a>> for ::super::SpecMethod
...
#[test]
fn test_unblocked_encodable_bytes_written_matches_len() {
    let payload: Unblocked = Default::default();
    let expected_len = ::Encodable::encoded_size(&payload);
    let mut writer = ::std::io::Cursor::new(Vec::with_capacity(expected_len));
    ::Encodable::write_encoded_to(&payload, &mut writer).unwrap();
    let payload = writer.into_inner();
...
} // impl From<Unblocked> for ClassMethod

impl From<Unblocked> for super::SpecMethod<'static> {
    fn from(from: Unblocked) -> Self {
        super::SpecMethod::Connection(from.into())
    } // fn default()
} // impl From<Unblocked> for ::super::SpecMethod
